...
say();
say(`    BUILD-2 HITTER TAIL (PINNED_HIT_TAIL) — ALSO SHIPPED, and also re-gated. An earlier draft of`);
say(`      this tool called it "dormant behind Tournament.hitTailCorrection". THAT WAS WRONG: it has`);
say(`      been STANDARD SCORING since 2026-07-17 (Derek's ruling 3 / plan §15.7 — universal`);
say(`      corrections ship ON BY DEFAULT with a global kill-switch), and hitTailCorrection is only an`);
say(`      override-OFF escape hatch that no tournament sets. A sweep that excluded it would not have`);
say(`      been a sweep of everything shipping, and its 7/7 gate record has the same clause-4 problem`);
say(`      as the HR ramp: it was established at the PRE-C1/C2' coordinate.`);
say();
